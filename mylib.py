...
def XY_order_cluster(label, district_label, order_group, traffic_group, dts):
    '''
    This funciton will not consider the first 3 time slot (or more), for all the days.
    It could be changed when test data set changes.
    '''
    ...
    timeslot.index = dts
    timeslot = timeslot.applymap(lambda x: {1.0: True, 0.0: False}[x])
    traffic_group_lst = pd.DataFrame()
    order_group_lst = pd.DataFrame()
    train = pd.DataFrame()
    test = pd.Series()
    for district in DISTRICTS:
        if (district in traffic_group and district_label.ix[district].values[0] == label):
            traffic_group_lst = [
                traffic_group[district].ix[dts - 1].rename(columns=lambda x: '1_' + x, index=lambda x: x + 1),
                traffic_group[district].ix[dts - 2].rename(columns=lambda x: '2_' + x, index=lambda x: x + 2),
                traffic_group[district].ix[dts - 3].rename(columns=lambda x: '3_' + x, index=lambda x: x + 3)]
            order_group_lst = [
                order_group[district].ix[dts - 1].rename(columns=lambda x: '1_' + x, index=lambda x: x + 1),
                order_group[district].ix[dts - 2].rename(columns=lambda x: '2_' + x, index=lambda x: x + 2),
                order_group[district].ix[dts - 3].rename(columns=lambda x: '3_' + x, index=lambda x: x + 3)]
            temp_train = pd.concat(traffic_group_lst + order_group_lst + [timeslot], axis=1)
            train = pd.concat([train, temp_train])
            train = train.fillna(0)
            test = pd.concat([test,
                              (order_group[district]['request'] - order_group[district]['answer']).\
                                ix[temp_train.index].fillna(0)])
...
def request_answer_count(order_group):
    ra_cnt = pd.DataFrame()
    order_group2 = copy.deepcopy(order_group)
    for district in DISTRICTS:
        order_group2[district].index = order_group2[district].index / 1000
        order_count = order_group2[district].groupby(order_group2[district].index ).sum()
        ra_cnt  = pd.concat([ra_cnt, order_count.loc[:, ['request', 'answer']].\
                               rename(columns = lambda x: x + '_' + district)], axis = 1)
    ra_cnt = ra_cnt.T
    request_index = filter(lambda x: x[:7] == 'request', ra_cnt.index)
    answer_index = filter(lambda x: x[:6] == 'answer', ra_cnt.index)
    ra_cnt = pd.concat([ra_cnt.ix[request_index].rename(columns = lambda x: 'request_' + str(x % 100), 
                                                         index = lambda x: x[8:]),
                                 ra_cnt.ix[answer_index].rename(columns = lambda x: 'answer_' + str(x % 100), 
                                                         index = lambda x: x[7:])], axis=1)
    return ra_cnt
...
class Search_Model(object):
    ...
    def fit(self, search_params, X, Y):
        ...
        print "Best Params:", self.best_estimator.get_params()
        print "Fit score:", self.best_estimator.score(X, Y)
